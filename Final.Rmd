...
We see that M16 and MP5 are very similar and are often misclassified for each other by our random forest model. We can choose to eliminate one or the other or we can try and find a variable to include in our model that is able to separate the two.

```{r}
Weapons %>% 
  filter(PrimaryWeapon == "M16" | PrimaryWeapon == "MP5") %>% 
  ggplot(aes(x = Eliminations, y = Damage, color = PrimaryWeapon)) +
  geom_point()
```
...
'Month' seems to be the primary separator in terms of these two weapons.

```{r}
Weapons %>% 
  filter(PrimaryWeapon == "M16" | PrimaryWeapon == "MP5") %>% 
  group_by(PrimaryWeapon, GameType) %>% 
  summarize(
    mean_Eliminations = mean(Eliminations),
...
Here we see that the weapon MP5 was only chosen in TDM game types. Including this variable may help our model differentiate between the two weapons. We can also remove previous variables included in our model that made seemingly no difference, i.e., Map1, Map2, and Choice.

```{r}
# New model with GameType included
set.seed(123)
rf2 <- randomForest(as.factor(PrimaryWeapon) ~ Eliminations + Deaths + TotalXP + Score + Damage + Month + GameType, data = Train, 
                    ntree = 500, mtry = 3, importance = TRUE)
set.seed(NULL)

#Obtain predicted probabilities - not need for confusion matrix but included to show we all get same probabilities with same seed
pred_prob2 <- predict(rf2, newdata = Test, type = "prob")

#To create the confusion matrix, we need to determine which class (No or Yes) is more likely. Although we could define a threshold and compare probabilities as we have done in the past, things are more challenging if y has more than 2 classes. So, let R pick for us.
pred_surv2 <- predict(rf2, newdata = Test, type = "response")

#Create confusion matrix
table(pred_surv2, Test$PrimaryWeapon)

#Calculate accuracy
mean(pred_surv2 == Test$PrimaryWeapon)

rf2$importance
```

We can use a feature selection selection model to give us an idea of which features are most important. Let's build a LASSO model to help us identify those variables most important to predicting PrimaryWeapon. 
...
# Build LASSO model
formula <- as.formula("PrimaryWeapon ~ Eliminations + Deaths + TotalXP + Score + Damage + Month + GameType")
...
We see that the month variable dominates the model, while interestingly enough 'Eliminations' and 'Deaths' are discarded by this LASSO model telling us that they may not be that important. Let's try the model again but without the 'Month' variable.
...
This may be too much regularization as only two variables are kept. Let's see which variables the mse lambda leaves us.
...
We'll remove 'Damage' from our RandomForest model.
...
# New model
set.seed(123)
rf3 <- randomForest(PrimaryWeapon ~ Month + Damage + Eliminations + Score + GameType + TotalXP + Deaths + Map1 + Map2 + Choice + FullPartial + XPType, data = Train, 
                    ntree = 500, mtry = 10, importance = TRUE)
set.seed(NULL)

#Obtain predicted probabilities - not need for confusion matrix but included to show we all get same probabilities with same seed
pred_prob3 <- predict(rf3, newdata = Test, type = "prob")

#To create the confusion matrix, we need to determine which class (No or Yes) is more likely. Although we could define a threshold and compare probabilities as we have done in the past, things are more challenging if y has more than 2 classes. So, let R pick for us.
pred_surv3 <- predict(rf3, newdata = Test, type = "response")

#Create confusion matrix
table(pred_surv3, Test$PrimaryWeapon)

#Calculate accuracy
mean(pred_surv3 == Test$PrimaryWeapon)
...
xvars <- c("Eliminations", "Deaths", "Score", "Damage")
...
accuracy <- sum(actual == predicted) / length(actual)  # calculate accuracy
accuracy
```
